...
@router.post("/{turma_id}/matricular", response_model=TurmaCompleta, summary="Matricular aluno logado em uma turma")
def matricular_aluno_turma(
    turma_id: int,
    session: SessionDependency,
    current_username = Depends(get_current_username) 
):
    # Assumindo que current_username é o ID do aluno
    return TurmaService.inclui_turma_no_aluno(session, int(current_username), turma_id)

@router.delete("/{turma_id}/desmatricular", response_model=TurmaCompleta, summary="Desmatricular aluno logado de uma turma")
def desmatricular_aluno_turma(
    turma_id: int,
    session: SessionDependency,
    current_username = Depends(get_current_username)
):
    return TurmaService.remove_turma_do_aluno(session, int(current_username), turma_id)

@router.post("/{aluno_id}/monitorar/{turma_id}", response_model=TurmaCompleta, summary="Tornar um aluno monitor")
...
@router.delete("/{aluno_id}/desmonitorar/{turma_id}", response_model=TurmaCompleta, summary="Remover monitor")
