...
@router.get("/username/{username}", response_model=UsuarioRead, summary="Obter um usuário por username", dependencies=[Depends(possui_permissao(["ADMIN"]))])
def get_usuario_by_username(username: str, session: SessionDependency):
    return UsuarioService.get_by_username(session, username)

@router.get("/{usuario_id}", response_model=UsuarioRead, summary="Obter um usuário por ID", dependencies=[Depends(possui_permissao(["ADMIN"]))])
def get_usuario_by_id(usuario_id: int, session: SessionDependency):
    return UsuarioService.get_by_id(session, usuario_id)

@router.delete("/{usuario_id}", summary="Deletar um usuário", dependencies=[Depends(possui_permissao(["ADMIN"]))])
def delete_usuario(usuario_id: int, session: SessionDependency):
    return UsuarioService.delete(session, usuario_id)

@router.get("/", response_model=list[UsuarioRead], summary="Obter todos os usuários", dependencies=[Depends(possui_permissao(["ADMIN"]))])
...
@router.put("/{usuario_id}", response_model=UsuarioRead, summary="Atualizar informações de um usuário", dependencies=[Depends(possui_permissao(["ADMIN"]))])
